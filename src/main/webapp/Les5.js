function initPage() {
	...
	// call holiday API
	setInterval(loadCountries(),5000);
	
	

	// call showWeather with the coordinates and city of IP API as parameters
	showWeather(Math.round(locatie_json.latitude), Math
			.round(locatie_json.longitude), locatie_json.city);
}
...
function showWeather(lat, lon, city) {
	...
	var weatherUrl = ("http://api.openweathermap.org/data/2.5/weather?lat="
			+ lat + "&lon=" + lon + "&APPID=" + openWeatherMapKey);
	...
	// save to HTML localstorage
	var now = new Date();
	window.localStorage.setItem(city, JSON.stringify(weather_json) + ",\"lastupdate\":"+now.toString().substring(16,24))
	console.log(window.localStorage)
	
//	var local_weather_json = JSON.parse(window.localStorage);
//	console.log(local_weather_json)
	...
	document.querySelector("#onder").innerHTML = d2.toString()
			.substring(15, 24);
}
...
//counter to only append the table one time
var count = 0;
...
function loadCountries() {
	...
	var countries_json = JSON.parse(getApi("restservices/countries"));
		...
		for (let x = 0; countries_json.length > x; x++) {
			...
			showWeather(countries_json[x]["latitude"], countries_json[x]["longitude"],
					countries_json[x]["capital"]);	
		
		
			
			tableRow.addEventListener('click', function() {
				showWeather(countries_json[x].latitude,
						countries_json[x]["longitude"],
						countries_json[x]["capital"])});
